...
export async function POST(request: Request) {
  ...
  const plus2Days = new Date(now)
  plus2Days.setDate(plus2Days.getDate() + 2)

  const plus4Days = new Date(now)
  plus4Days.setDate(plus4Days.getDate() + 4)

  // Query active trial subscriptions.
  // The farms relationship returns a single farm object.
  const { data: rawTrials, error: queryError } =
    await supabase
      .from("subscriptions")
      .select(`
        farm_id,
  ...
  const trials =
    (rawTrials ?? []) as unknown as SubscriptionRow[]

  // Collect all owner IDs for a single bulk profile lookup.
  const ownerIds = [
    ...new Set(
      trials
        .map((trial) => trial.farms?.owner_id)
        .filter(
  ...
  // profiles.email is the canonical email source.
  ...
  // Determine which lifecycle email each trial requires.
  type Bucket = {
    row: SubscriptionRow
    type: TrialEmailType
  }
  ...
  await Promise.all(
    buckets.map(async ({ row, type }) => {
      ...
      // Expiry status is persisted independently of email delivery.
  ...
  return NextResponse.json({
    ...
    breakdown: {
      trial_expired: buckets.filter(
        (bucket) =>
          bucket.type === "trial_expired"
      ).length,

      trial_1_day: buckets.filter(
        (bucket) =>
          bucket.type === "trial_1_day"
      ).length,

      trial_3_days: buckets.filter(
        (bucket) =>
          bucket.type === "trial_3_days"
      ).length,
    },

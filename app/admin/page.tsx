...
export default function AdminOverviewPage() {
  ...
  async function loadOverview() {
    try {
      ...
      if (!response.ok || !result.success) {
        setError(result.error || "Unable to load admin overview.");
        return;
      }

      setData(result);
    } catch (err) {
      console.error("Admin overview loading error:", err);
      setError("Unable to load the Admin Control Centre.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customers.filter((customer) => {
      const matchesSearch =
        !query ||
        customer.farm_name?.toLowerCase().includes(query) ||
        customer.owner_email?.toLowerCase().includes(query) ||
        customer.owner_name?.toLowerCase().includes(query) ||
        customer.plan?.toLowerCase().includes(query);

      const matchesStatus =
        statusFilter === "all" ||
        (customer.subscription_status || "").toLowerCase() === statusFilter;
        ...
        <header className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Central visibility of customers, trials, subscriptions,
...
<KpiCard
  ...
  onClick={() => {
    document
      .getElementById("customer-overview")
      ?.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
...
<KpiCard
  ...
  value={summary?.expiringTrials ?? 0}
  detail="Needs attention"
  accent="amber"
  onClick={() => router.push("/admin/expiring")}
/>

<KpiCard
  label="Expired Trials"
  value={summary?.expiredTrials ?? 0}
  detail="Trial ended"
  accent="red"
...
<KpiCard
  ...
  value={formatNumber(summary?.revenue ?? 0)}
...
<tr
  ...
  title={`Open ${customer.farm_name || "customer"} details`}
>
                      <td className="whitespace-nowrap px-5 py-4">
                        <div className="font-semibold text-slate-900">
                          {customer.farm_name || "Unnamed farm"}
                        </div>
                        <div className="mt-0.5 text-xs text-slate-500">
                          {customer.farm_type || "Poultry"}
                          {customer.currency
                            ? ` � ${customer.currency}`
                            : ""}
                        </div>
                      ...
                      <td className="whitespace-nowrap px-5 py-4">
                        ...
                        >
                          {statusLabel(customer.subscription_status)}
                        </span>
                      </td>

                      <td className="whitespace-nowrap px-5 py-4 text-sm text-slate-500">
                        {customer.subscription_status === "trial"
                          ? formatDate(customer.trial_end)
                          : formatDate(customer.next_billing_date)}
                      </td>
                    </tr>
                  ))}
